...
export function Card({
  ...
  const defaultProps = {
    className: cn(
      "relative flex flex-col rounded-md border bg-card not-dark:bg-clip-padding text-card-foreground before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-2xl)-1px)]",
      className,
    ),
    "data-slot": "card",
  };
...
export function CardHeader({
  ...
  const defaultProps = {
    className: cn(
      "grid auto-rows-min grid-rows-[auto_auto] items-start gap-1.5 p-6 in-[[data-slot=card]:has(>[data-slot=card-panel])]:pb-2 has-data-[slot=card-action]:grid-cols-[1fr_auto]",
      className,
    ),
